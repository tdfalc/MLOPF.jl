...
function generate_sample(network::Dict{String,Any}, alpha::Float64; id::Int = 1, max_iter::Int = 100, nmo::Bool = False)
    ...
    while !is_feasible
        let network = deepcopy(network)
            ...
            # Solve the new AC-OPF problem and check feasibility based on the terminiation status of Ipopt.
            result = solve_acopf(network, max_iter)
...
"Instantiate power model and solve AC-OPF problem using the Ipopt algorithm."
function solve_acopf(network::Dict{String,Any}, max_iter::Int)
    ...
    result["binding_status"] = MLOPF.binding_status(pm)
    result["load"] = Dict("pd" => MLOPF.get_load_pd(network), "qd" => MLOPF.get_load_qd(network))
    return result
end
